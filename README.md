...
Commands are below to build and run the docker image (from the directory where docker is file placed)
	...
	II) docker run -d --name station_management_container --network=station_management_db_network -e DB_HOST=postgres -p8081:8080 charging-station-management:latest 
